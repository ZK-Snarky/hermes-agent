...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions():
        ...
        for chunk in stream:
            ...
            # Accumulate text content. If this chunk is associated with tool
            # calls, keep the content in the final assistant message but do not
            # stream it to user-facing platforms. Some providers emit planning
            # text like "Need load skill..." in the same turn as tool calls;
            # iMessage/Photon cannot retract streamed previews.
            chunk_has_tool_delta = bool(delta and getattr(delta, "tool_calls", None))
            if delta and delta.content:
                content_parts.append(delta.content)
                if not tool_calls_acc and not chunk_has_tool_delta:
                    _fire_first_delta()
