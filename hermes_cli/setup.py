...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    Used after legacy migration to detect which sections can be skipped.
...
def _offer_legacy_migration(hermes_home: Path) -> bool:
    """Legacy migration is disabled in this local runtime."""
    return False
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        # missing items" flow (useful after a partial legacy migration
        ...
        # Legacy migration prompts are disabled for Seb's local runtime.
        migration_ran = False
    ...
    if migration_ran:
        print()
        print_info("Settings were imported from a legacy agent.")
