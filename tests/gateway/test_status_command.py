...
@pytest.mark.asyncio
async def test_first_run_photon_skips_home_channel_onboarding(monkeypatch):
    import gateway.run as gateway_run

    photon = Platform("photon")
    session_entry = SessionEntry(
        session_key=build_session_key(_make_source(photon)),
        session_id="sess-1",
        created_at=datetime.now(),
        updated_at=datetime.now(),
        platform=photon,
    ...
    runner = _make_runner(session_entry, platform=photon)
    runner.session_store.load_transcript.return_value = []
    runner.session_store.has_any_sessions.return_value = False
    runner._run_agent = AsyncMock(
        return_value={
    ...
    result = await runner._handle_message(_make_event("hello", platform=photon))

    assert result == "ok"
    runner.adapters[photon].send.assert_not_awaited()
